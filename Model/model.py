...
def load_model(path=os.path.join(default.WEIGHTS_PATH,"checkpoint1.pth")):
    ...
    model.load_state_dict(torch.load(path,map_location=torch.device(default.DEVICE)))
    model.eval()
    return model
...
def generate_text(model,initial_text,length):
    ...
    for i in range(length):
        ...
        pred, (state_h, state_c) = model(torch.tensor([ind],device=default.DEVICE).view(1,-1), (state_h, state_c))
    return output
